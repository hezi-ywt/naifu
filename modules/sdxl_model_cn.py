...
# define the LightningModule
class StableDiffusionModelCN2(pl.LightningModule):
    def __init__(self, config, device) -> None:
        super().__init__()
        self.config = config
        self.target_device = device
    ...
    def build_models(self, init_unet=True, init_vae=True, init_conditioner=True):
        ...
        self.text_encoder.training=False
        self.text_encoder_2.training=False
        # if self.vae.hasAttribute(name="eval"):
        #     self.vae.eval()
        # if self.unet.hasAttribute(name="eval"):
        #     self.unet.eval()
        # if self.text_encoder.hasAttribute(name="eval"):
        #     self.text_encoder.eval()
        # if self.text_encoder_2.hasAttribute(name="eval"):
        #     self.text_encoder_2.eval()
        # if self.tokenizer.hasAttribute(name="eval"):
        #     self.tokenizer.eval()
        # if self.tokenizer_2.hasAttribute(name="eval"):
        #     self.tokenizer_2.eval()

        # self.vae.requires_grad(False)
        # self.unet.requires_grad(False)
        # self.text_encoder.require_grad(False)
        # self.text_encoder_2.require_grad(False)
        # self.vae.eval()
        # self.unet.eval()
        # self.tokenizer.eval()
        # self.tokenizer_2.eval()
    ...
    def init_model(self):
        advanced = self.config.get("advanced", {})
        sd = load_torch_file(self.config.trainer.get("sdxl_model_path"), self.target_device)
    ...
    #                 iids_list.append(ids_chunk)
    #         print(ids_chunk)
    #         print(ids_chunk.shape)
    #         input_ids = torch.stack(iids_list)  # 3,77
    #     return input_ids
    
    @torch.no_grad()
    def process_input_ids(self, input_ids, tokenizer):
    ...
    def encode_batch(self, text, text_encoder, tokenizer):
        max_token_length = self.max_token_length
        ...
        # b,n,77
        input_ids = torch.stack([self.process_input_ids(batch, self.tokenizer) for batch in batch_encoding]).to(self.device)
    ...
    def encode_text(self, prompt):
        ...
        print(input_ids2)
        b_size = len(prompt)
        input_ids1 = input_ids1.reshape((-1, tokenizer.model_max_length))  # batch_size*n, 77
        
        input_ids2 = input_ids2.reshape((-1, tokenizer_2.model_max_length))  # batch_size*n, 77
        # input_ids: b,n,77
        # pool2 = enc_out["text_embeds"]
        unwrapped_text_encoder2 = text_encoder_2 
        pool2 = self.pool_workaround(unwrapped_text_encoder2, enc_out["last_hidden_state"], input_ids2, tokenizer_2.eos_token_id)
